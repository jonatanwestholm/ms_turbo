...
class Game:
    def __init__(self, width, height, num_mines):
        self.board, node_0 = get_workable(height, width, num_mines)
        self.solution = Board(height, width, num_mines)
        self.visible_tiles = set()
        self.solution.sweep(self.board, node_0)
        self.solution.exhaust_1(self.board)

    def get_new_tiles(self):
        tiles = set()
        for node in self.solution:
            if self.solution.nodes[node]["mine"] is not None and node not in self.visible_tiles:
                self.visible_tiles.add(node)
                y, x = node
                mine = self.solution.nodes[node]["mine"]
                if mine:
                    adj = 9
                else:
                    adj = self.solution.nodes[node]["adj"]
                tiles.add((x, y, adj))
        print("revealed tiles:", len(tiles))
        return tiles
